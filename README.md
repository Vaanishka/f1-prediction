...
|-----------|----------------|--------|
| Grid position impact on race | Grid_Position + Overtake_Factor + Projected_Stability | ✅ |
| Tire compound comparison | Tire_Adjusted_Pace (normalizes Leclerc/Hards vs Alonso/Softs) | ✅ |
| Tire degradation | Tire_Tax + Tyre_Age + Total_Tire_Degradation | ✅ |
| Fuel load intelligence | Fuel_Adjusted_Pace + session-aware fuel estimation | ✅ |
| Qualifying vs race fuel | Is_Qualy flag + fuel-adjusted normalization | ✅ |
| FP2 sandbagging detection | Sandbagging_Score (CV-based detection) | ✅ |
| 2026 technical regulations | Drag_Coefficient, Override_Boost, Deployment_Error, Power_Clipping | ✅ |
| Driver consistency | Driver_Consistency + Speed_CV + Risk_Factor_Pct | ✅ |
| 5-fold cross-validation | Proper CV by race (no data leakage) | ✅ |
| XGBoost algorithm | XGBRegressor with optimized hyperparameters | ✅ |
| Memory efficiency | Aggregated data (no iterator needed for 5 races) | ✅ |
...
f1_pred/
├── coefficient/
│   ├── rules.json              # Your rules configuration
│   └── track_dna.json          # Track characteristics
├── models/
│   ├── training_data.csv       # Generated by load_data_aggregated.py
│   ├── xgboost_model_5fold.pkl # Trained model
│   ├── scaler.pkl              # Feature scaler
│   ├── driver_encoder.pkl      # Driver encoder
│   ├── race_encoder.pkl        # Race encoder
│   ├── feature_names.json      # Feature list
│   ├── model_metadata.json     # Model info
│   ├── cv_results.json         # 5-fold CV metrics
│   └── *.png                   # Visualizations
└── scripts/
    ├── load_data_aggregated.py # Step 1: Data collection
    ├── train_with_5fold_cv.py  # Step 2: Model training
    └── predict.py              # Step 3: Make predictions
```

---

## 🚀 Complete Pipeline - 3 Simple Steps
...
**What it does:**
- Processes 5 races × 3 sessions (FP2, Q, R) = 15 sessions
...
- Implements **proper 5-fold CV by race**:
  ```
  Fold 1: Train[Hungary, Belgium, Singapore, Silverstone] → Test[Monza]
  Fold 2: Train[Monza, Belgium, Singapore, Silverstone] → Test[Hungary]
  Fold 3: Train[Monza, Hungary, Singapore, Silverstone] → Test[Belgium]
  Fold 4: Train[Monza, Hungary, Belgium, Silverstone] → Test[Singapore]
  Fold 5: Train[Monza, Hungary, Belgium, Singapore] → Test[Silverstone]
  ```
...
**Output Example:**
```
🏆  RACE POSITION PREDICTIONS - MONACO GRAND PRIX
======================================================================

   Model Accuracy: ±2.3 positions (from 5-fold CV)
...
Leclerc on Hards:  320 kph raw → 329 kph adjusted (320 / 0.97)
Alonso on Softs:   320 kph raw → 311 kph adjusted (320 / 1.03)
...
41. Clipping_Pct
42. Avg_Terrain_Strain (elevation × speed)
...
48. Grid_X_Overtake (grid position × overtake difficulty)
49. Avg_Tire_X_Fuel (tire grip × fuel load)
50. Avg_Pace_X_Consistency (pace × consistency)
...
## 🎯 How Your Objectives Are Captured

### 1. **Grid Position → Race Impact**
```python
...
```
**Model learns:** Starting P1 at Monaco ≠ Starting P1 at Monza
...
Conclusion: Equal pace ❌

# With Tire_Adjusted_Pace (CORRECT):
Leclerc (Hards): 320 kph → 329 kph adjusted
Alonso (Softs):  320 kph → 311 kph adjusted
Conclusion: Leclerc is faster! ✅
```
...
```python
Driver A: [1:22.1, 1:22.3, 1:22.2] → Low variance → Genuine pace
Driver B: [1:24.5, 1:21.8, 1:23.2] → High variance → Sandbagging

Sandbagging_Score = CV% × 5
...
Problem: Same driver's laps appear in both train and test → DATA LEAKAGE!
```

**Our Approach: 5-Fold by Race (CORRECT):**
```
Fold 1: Train on 4 races → Test on Monza
Fold 2: Train on 4 races → Test on Hungary
Fold 3: Train on 4 races → Test on Belgium
Fold 4: Train on 4 races → Test on Singapore
Fold 5: Train on 4 races → Test on Silverstone

Average CV score = Expected performance on unseen tracks
```

**Benefits:**
- ✅ No data leakage (entire races separated)
- ✅ Tests generalization to different tracks
- ✅ Mimics real use: "Predict next race from past races"
- ✅ Identifies which tracks are harder to predict
...
- Bar charts showing Test MAE and R² for each fold
...
| CV R² | > 0.60 | Explains 60%+ of position variance |
| Error Distribution | Centered at 0 | No systematic bias |
| Feature Importance | Grid_Position in top 3 | Model logic makes sense |

**Performance Tiers:**
- **Excellent** ⭐⭐⭐: MAE < 2.0, R² > 0.75
- **Very Good** ⭐⭐: MAE 2.0-2.5, R² 0.70-0.75
- **Good** ⭐: MAE 2.5-3.0, R² 0.60-0.70
- **Fair**: MAE 3.0-4.0, R² 0.50-0.60
- **Needs Work** ⚠️: MAE > 4.0, R² < 0.50
...
### **What You Built:**
✅ 60+ features capturing all your objectives
✅ Aggregated data (no overfitting, proper CV possible)
✅ 5-fold cross-validation by race (proper validation)
✅ XGBoost model (no iterator needed for your dataset size)
✅ Complete pipeline (data → train → predict)
...
### **Next Steps:**
1. Run `load_data_aggregated.py` → Get training data
2. Run `train_with_5fold_cv.py` → Train and evaluate
...
- Tire compound intelligence ✅
- Fuel load normalization ✅
